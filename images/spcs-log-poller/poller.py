...
OBSERVE_LOKI_URL = os.environ.get(
    "OBSERVE_LOKI_URL", ""
)  # e.g. https://<cid>.collect.observeinc.com/v1/http
OBSERVE_DATASTREAM_TOKEN = os.environ.get("OBSERVE_DATASTREAM_TOKEN", "")
...
def push_to_loki(logs: list[dict]) -> dict:
    """Push log entries to Loki via HTTP push API. Returns the payload for reuse."""
    if not logs:
        return {}
...
def push_to_observe(payload: dict) -> None:
    """Push the same Loki-format payload to Observe's /v1/loki endpoint."""
    if not payload or not OBSERVE_LOKI_URL or not OBSERVE_DATASTREAM_TOKEN:
        return

    try:
        resp = requests.post(
            f"{OBSERVE_LOKI_URL}/loki/api/v1/push",
            json=payload,
            headers={
                "Content-Type": "application/json",
                "Authorization": f"Bearer {OBSERVE_DATASTREAM_TOKEN}",
            },
            timeout=10,
        )
        if resp.status_code not in (200, 204):
            log.warning("Observe push failed: %d %s", resp.status_code, resp.text[:200])
    except Exception as exc:
        log.warning("Observe push error: %s", exc)
...
def poll_once(conn: snowflake.connector.SnowflakeConnection) -> int:
    ...
    payload = push_to_loki(logs)
    push_to_observe(payload)
    if _is_first_poll:
...
def main() -> None:
    ...
    if OBSERVE_LOKI_URL and OBSERVE_DATASTREAM_TOKEN:
        log.info("Observe dual-write enabled: %s", OBSERVE_LOKI_URL)
