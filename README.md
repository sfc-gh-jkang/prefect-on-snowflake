...
- **Duplicate `external_labels` across VM agents:** Each VM running prometheus-agent MUST have unique `WORKER_LOCATION` and `WORKER_POOL` values (set in `.env`). If two VMs share the same labels, they produce identical time series — the SPCS Prometheus receiver rejects the duplicates as "Out of order sample" errors (hundreds of thousands per minute). Fix: ensure every VM has a unique `(WORKER_LOCATION, WORKER_POOL)` pair, e.g. `aws-us-west-2`/`aws-pool` and `aws-us-west-2-backup`/`aws-pool-backup`. After changing labels, clear the Prometheus WAL (`docker volume rm` the agent data volume) to flush stale series, then restart.
